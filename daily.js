...
const apparitionDaily = {
    ...
    "process": async({utils,fields,addFeat,addItem,messages, actor})=>{
        ...
        for (const field in fields){
            if (field == 'entryFocus' || field == 'entrySpontaneous') { continue; }
            ...
            for (const spell of spells){
                const index = arraySpellSource.findIndex((e) => e.system.slug == spell.object.system.slug)
                if (index >= 0) { 
                    arraySpellSource[index].flags['pf2e-animist-spell'].source.push(source.system.slug);
                    continue ;
                }

                const spellSource = await utils.createSpellSource(spell.object.uuid);
            ...
            const vesselSpell = await game.modules.get('pf2e-animist-spell').api.parseApparitionVessel(uuid);
